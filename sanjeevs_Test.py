...
def main():
    ...
    print('\nvalidation started...')
    predict(x_train, y_train, nn, l_in, l_out, y_trg_test, y_pred)
    #predict(x_valid, y_valid, nn, l_in, l_out, y_valid_test, y_pred)
    ...
    tp = 0
    tot_cnt = len(y_trg_test)
    for j in range(0, len(y_trg_test)):
        if y_trg_test[j] == y_pred[j]:
            tp += 1
    print('Training Precision = {0:.3f}'.format(tp / (1. * tot_cnt)))

    '''tp = 0
    tot_cnt = len(y_valid_test)
    for j in range(0, len(y_valid_test)):
        if y_valid_test[j] == y_pred[j]:
            tp += 1
    print('Validation Precision = {0:.3f}'.format(tp / (1. * tot_cnt)))'''
    print('\ntesting started...')
    y_pred = []
    predict(x_test, y_test, nn, l_in, l_out, y_test_test, y_pred)
    tp = 0
    tot_cnt = len(y_test_test)
    for j in range(0, len(y_test_test)):
        if y_test_test[j] == y_pred[j]:
            tp += 1
    print('Test Precision = {0:.3f}'.format(tp / (1. * tot_cnt)))
